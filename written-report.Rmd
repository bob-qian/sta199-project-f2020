...
To begin with, we calculate summary statistics for stop rate based on race in 2010:
...
From the table, it appears that black people have the highest stop rate among races in Durham in 2010. This rate is 2.5 times higher than the rate for white people in Durham in 2010.

Then, we use census data to compare the expected proportion of people stopped by race to the observed proportion of people stopped by race in Durham.
...
According to the chart, a significantly greater proportion of black people were stopped compared to what was expected based on the proportion of black people in Durham County in the 2010 census.
...
ggplot(combined2, aes(x = O, y = proportion, fill = subject_sex)) + 
  ...
  labs(title = "Observed vs Expected Proportions of People Stopped by Sex", 
       ...
       y = "Proportion", 
       fill = "Subject Sex") +
  scale_fill_manual(values = c("#ede3da", "#31394d")) +
  theme(aspect.ratio = 0.6)
```

According to the chart, a significantly greater proportion of males were stopped compared to what was expected based on the proportion of males in Durham County in the 2010 census.

Then, we visualize a segmented bar graph with the proportion of citations based on race below. 
...
To answer the first part of our research question on the relationship between race and sex and the likelihood of being stopped in traffice by police, we conducted a hypothesis test using a simulation-based approach. We created null distributions for the proportion of stopped drivers separately based on race and on sex, and then compare these proportions with the census data on proportions of race and sex in Durham to test if their is any statistical difference.
...
We have also learned that black people are disproportionately more likely to be stopped for a traffic violation, as opposed to their makeup of the total Durham population. However, black people were not the most likely to receive a citation upon being stopped. We found that Hispanics were the most likely to receive a citation upon being stopped. It is unclear as to why this occurs because we only have observational data, which is a limitation. Because we do not have randomization or experimental design, our data analysis only provides evidence for the observation that black people are more likely to be stopped than white people. Though we cannot conclude any cause-and-effect relationship between race and the likelihood of being stopped by police, our data is consistent with past literature. For instance, our analysis is consistent with the discussion on racial profiling and pretext stops in Michelle Alexander's The New Jim Crow: Mass Incarceration in the Age of Colorblindness. Nonetheless, we only have evidence that suggests statiscally significant differences in proportion of black drivers being stopped (as opposed to their population percentage) and white drivers being stopped (as opposed to their population percentage). This concludes our hypothesis that race and the likelihood of being stopped by police in traffic in Durham are related as well as disproving our hypothesis that sex has no significant relationship with being stopped by police in traffic Durham. We also proved our hypothesis that younger people are more likely to receive traffic citations upon being stopped. 

Appendix 2 further explores the trends of the relationship between Traffic stops, sex, and race by time. Overall, these visualizations show that policing trends have not drastically changed over the time period from 2002 to 2015. 

For most races, there is very little change in data between citations being given by sex. However, significantly more black females are given citations than their male counterparts. This could possibly show unspoken discrimination against black females. However, with our observational data limitations, it is unknown if there are any lurking or unknown variables. 
