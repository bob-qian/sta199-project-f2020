...
4. Holding age constant, we expect the odds that a white male  will receive a citation upon being stopped by police in Durham County to be `r exp(0.03228)` times the odds that a white female will receive a citation upon being stopped. This coefficient, however, is not statistically significant at the 1% level (p-value > 0.01).
...
2. Linearity - Below, we have depicted scatterplots of the relationship between age and the log-odds of receiving a citation, controlling for race and sex. The Linearity Assumption is met because there is a roughly linear relationship between the age of a subject and the log-odds of receiving a citation when other predictors (race and sex in this context) are held constant.

```{r emplogits, fig.width = 4, fig.height = 4}
emplogitplot2(citation_issued ~ subject_age +subject_sex, 
              data = police, ngroups = 10, putlegend = "bottomleft", 
              main = "Empirical Logits vs Age by Sex")
```
```{r emplogits2, fig.width = 4, fig.height = 4}
emplogitplot2(citation_issued ~ subject_age + subject_race, 
              data = police, ngroups = 10, 
              main = "Empirical Logits vs Age by Race")
#legend removed bc it covers the graph
```
