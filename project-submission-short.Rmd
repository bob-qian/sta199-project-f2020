...
As a preliminary check to decide if further investigation is warranted, we first conducted a chi-squared test for independence. With a p-value of roughly 0, we found sufficient evidence to indicate the existence of an association between race and citation issued (See Appendix 5). We then created a logistic regression model to quantify the effect of race (adjusted for other demographic variables such as sex and age) on log-odds of receiving a citation. The logistic regression model contains four predictors: The subject's race, the subject's age, the subject's sex, and an interaction variable between a subject's race and subject's sex (See Appendix 1 for the reasoning behind this logistic regression model and Appendix 3 for the checking of conditions). To determine statistical significance of coefficients, we set the alpha level to 0.01.
...
Another crucial limitation is that we are unable to establish if the trends we found were causally related. We are only able to take note of trends and cannot isolate causation; thus, our data does not provide conclusive evidence of racial discrimination by police.
